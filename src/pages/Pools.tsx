...
const Pools = () => {
  ...
  // Build token universe dynamically: native + indexed + user-added custom tokens
  const allTokens = useMemo(
    () => [NATIVE_TOKEN, ...Object.values(TOKENS), ...customTokens],
    [customTokens]
  );

  // Generate all unique pairs for the current token universe
  const allTokenPairs = useMemo(
    () =>
      allTokens.flatMap((tokenA, i) =>
        allTokens
          .slice(i + 1)
          .map((tokenB) => [tokenA, tokenB] as [Token, Token])
      ),
    [allTokens]
  );

  // 1. Get pair addresses for all possible token pairs
  const { data: pairAddressesData, isFetched: arePairAddressesFetched } =
    useReadContracts({
      ...
      query: { enabled: isConnected && allTokenPairs.length > 0 },
    });
      ...
      arePairAddressesFetched && pairAddressesData
        ? allTokenPairs
            ...
            .filter(
              (p) =>
                p.pairAddress &&
                p.pairAddress !== "0x0000000000000000000000000000000000000000"
            )
        : [],
    [arePairAddressesFetched, pairAddressesData]
  );

  const { data: lpBalancesData, isFetched: areLpBalancesFetched } =
    useReadContracts({
  ...
  const renderContent = () => {
    ...
    if (isLoading) {
      return (
        <p className="text-center text-gray-600">Loading your positions...</p>
      );
    }
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-lg text-gray-800">
                    {position.tokenA.symbol === "BLOCX"
                      ? "ETH"
                      : position.tokenA.symbol}
                    /
                    {position.tokenB.symbol === "BLOCX"
                      ? "ETH"
